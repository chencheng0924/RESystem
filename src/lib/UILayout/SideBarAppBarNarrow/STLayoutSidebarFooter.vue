...
<template>
    <div class="py-[12px] px-[8px] shadow-[0_5px_15px_0_rgba(0,0,0,0.05)] border-t-[1.5px] border-foneBorder" >
        <div class="mb-1">
            <nav>
                <ul class="layout-menu">
                    <li role="menuitem" class="flex flex-col mt-1" v-for="menu in systemMenus" >
                        <SidebarMenu :menuItem="menu" @event-action-btn="menuBtn" @event-action-new-btn="menuNewBtn"></SidebarMenu>
                    </li>
                </ul>   
            </nav>
        </div>
       
        <div class="flex" :class="{ 'justify-center': menuStore.isMenuCollapsed, 'justify-between': !menuStore.isMenuCollapsed }">
