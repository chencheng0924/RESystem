...
import STProjectCards from "@/components/smartcityui/STProjectCards.vue";


export class PageSectionProjectCard extends PageSection implements IPageCustom {
    ...
    // 取得元件實例
    getComponent() {
        return STProjectCards;
    }
    // section 轉 元件
    toComponent(self: PageBuilder, view: PageView, sec: PageSection): PageSection {

        let AgentCardSec = sec as PageSectionProjectCard;
        AgentCardSec.Component = STProjectCards
        ...
        AgentCardSec.Events = {
            ...
            eventActionCard: async (item: STCardItem) => {
                ...
                let promptSec = sec as PageSectionProjectCard;
