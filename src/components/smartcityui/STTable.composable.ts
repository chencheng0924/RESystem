...
export class useTableStyle {
    ...
    public getDataTableStyleOption() {
        return {
            bodyrow: ({ context, props }) => ({
                class: [
                    { '!bg-fonePrimaryBg ': context.selected }
                ],
            }),
            column: {
                headerCheckbox: {
            ...
            rowexpansioncell: ({ context, props }) => {
                return {
                    class: [
                        { 'p-0 border-none': props.rowData.pluginMethodParameters.length == 0 }
                    ],
                };
            },
        }
    ...
    private getPaginatorStyleOption() {
        return {
            ...
            root: {
                class: ['bg-transparent', 'mobile:px-2', 'miniMobile:!px-0', '!justify-end']
            },
...
export class useTable {
    ...
    // public searchDataList?: Ref<Array<searchData>>
    // public searchDataParams?: Ref<SearchDataParams>
    ...
    public checkActionEnable(item: STDataTableAction) {
        if (item.Id == 'delete') {
            if (this.selectedProduct.value.length > 0) return false
            return true
        } else {
            return !item.Enable
        }
