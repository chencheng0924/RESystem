...
const eventFunction=(eventName,data)=>{
  ...
  else if(eventName ==PageEventEnum.PageSecUpdateByPageSection )
  {
    let path = data as string;
    controller.updatePageSection(controller.pageView.value , path);

  }
  else if(eventName ==PageEventEnum.PageGoBack )
  {
   router.back();
  }
...
<template>
  ...
  <div class="flex justify-between w-full pt-2 !rounded-[6px]"
    :style="{ '--panel-height': 'calc(100vh - 68px - 65px)'  }">
    <Splitter class="w-full h-full" :key="splitterKey" >    
      <SplitterPanel class="flex items-center justify-center relative" :size="50">
            ...
            <div class="w-full flex flex-col h-full">
              ...
              <div class="flex !h-[var(--panel-height)]">
                    ...
                    <div class="flex flex-col !h-[var(--panel-height)] overflow-y-scroll w-full !bg-foneBg">
                      <div v-for="(leftSec, idx) in controller.getEntitySectionByLeft()" class="flex flex-col gap-[1rem]">
 
                         <div v-if="leftSec.IsPanel == false" class="h-full">
                          <div class="w-full h-full" :class="leftSec.getClass()">
                            <div class="text-center md:text-left flex align-items-center h-full" :key="leftSec.Id">
                              <component v-memo="[leftSec.Id, leftSec.Props, leftSec.Attrs, leftSec.Events]" 
                                :is="controller.getComponent(leftSec)" 
                                v-bind="{ ...leftSec.Props, ...leftSec.Attrs }"  v-on="{ ...leftSec.Events }" />
                            </div>

                          </div>
                        </div>
                        <div :key="leftSec.Id" v-else class="">

                           <Panel toggleable :pt="leftSec.penalPt">
                              <template #header key="header">
                                    <div class="flex items-center gap-2">
                                        <div class="w-[4px] h-[16px] bg-fonePrimaryMain"></div>
                                        <h4 class="font-bold">{{ t(leftSec.Title) }}</h4>
                                    </div>
                              </template>
                              <template #toggleicon="soltProperitys" >
                             
                                <div v-if="soltProperitys.collapsed">
                                  <Icon name="ic_arrow_down" custom-class="!w-[18px] !h-[18px] !text-foneTextLevel1"></Icon>
                                </div>
                                <div v-else>
                                  <Icon name="ic_arrow_up" custom-class="!w-[18px] !h-[18px] !text-foneTextLevel1"></Icon>
                                </div>

                              </template>

                              <component
                                v-memo="[leftSec.Id, leftSec.Props, leftSec.Attrs, leftSec.Events]"
                                :id="leftSec.Id"
                                :is="controller.getComponent(leftSec)"
                                v-bind="{ ...leftSec.Props, ...leftSec.Attrs }"
                                :model-value="leftSec.Props?.value" v-on="{ ...leftSec.Events }" />
                            </Panel>
       ...
       <SplitterPanel class="flex items-center justify-center relative h-full" :size="50">
      
        <!-- 這邊div的高度會影響 ListView 的 lazy loading -->
        <div class="bg-crmBgLevel1 h-[calc(100vh-68px-65px)] w-full overflow-y-scroll"
        :class="{ '!h-[calc(100vh-68px-65px)]': route.path.includes('AIAgentEdit') }">
          <div
