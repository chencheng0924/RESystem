...
import type { ILatLng, IMarker, IPolyLine, IInfoWindow, IBounds } from '../imap';
...
export class GMarker implements IMarker {
    public id?: string;
    ...
    showAlternateRoute(e) {
        ...
        console.warn('controlTrigger', controlTrigger);
        console.warn('googleMapStore.mapControllerList', googleMapStore.mapControllerList);

        let cmd = new CommandFactory();
        cmd.run(`Map showRoute routeId="${routeId}" openSubRoute="${true}" dirction=${direction}`, '')

        // if (controlTrigger) {
        //     let cmd = new CommandFactory();
        //     cmd.run(`Map showRoute routeId="${routeId}" openSubRoute="${true}" dirction=${controlTrigger.dirction}`, '', false)
