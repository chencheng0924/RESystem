...
} from './A2AMModel'; // Assuming schema.ts is in the same directory or appropriately pathed
...
export class A2AClient {
     ...
     */
    constructor(agentBaseUrl: string, headers?: any) {
    ...
    private async _fetchAndCacheAgentCard(): Promise<AgentCard> {
        const agentCardUrl = `${this.agentBaseUrl}/.well-known/agent-card.json`;
        let hd = { 'Accept': 'application/json' };
    ...
    private async _postRpcRequest<TParams, TResponse extends JSONRPCResponse>(
        ...
        if (this.headers != null) {
            hd = Object.assign(hd, this.headers);
        }
     ...
     */
    public async *sendMessageStream(params: MessageSendParams, method: string = 'POST', headers?: any): AsyncGenerator<A2AStreamEventData, void, undefined> {
